...
const DEFAULT_IGNORE_PATTERNS = ['.claude/**'];
...
export function getChangedFiles(projectRoot) {
  ...
  return [...new Set([...unstaged, ...staged, ...untracked].map(normalizeRelativePath).filter(Boolean))]
    .filter((file) => !matchesAnyPattern(file, DEFAULT_IGNORE_PATTERNS))
    .sort();
}
